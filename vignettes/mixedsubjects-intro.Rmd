...
fit <- msd_estimate(Y ~ D | S0 + S1, data = example_data, estimator = "dip_pp")
fit
```
...
```{r}
fit_dip <- msd_estimate(Y ~ D | S0 + S1, data = example_data, estimator = "dip")
fit_dt_dip <- msd_estimate(Y ~ D | S0 + S1, data = example_data, estimator = "dt_dip")
...
```{r}
msd_select(Y ~ D | S0 + S1, data = example_data)
```
